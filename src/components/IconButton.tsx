...
const IconButton: React.FC<IconButtonProps> = ({
  ...
  return (
    <ButtonWrapper onClick={onClick} aria-label={tooltip ?? alt} title={tooltip ?? alt}>
...
const Label = styled.span<{ $direction: "row" | "column" }>`
  font-size: 14px;
  text-align: center;
...
type IconButtonProps = {
  ...
  tooltip?: string;
  onClick: () => void;
  showLabel?: LabelPosition; // "none" | "side" | "below"
};
...
const IconButton: React.FC<IconButtonProps> = ({
  ...
  return (
    ...
    >
      <Icon src={icon} alt={alt} />
      {shouldRenderLabel && <Label $direction={direction}>{label}</Label>}
      {tooltip && <Tooltip className="tooltip">{tooltip}</Tooltip>}
    </ButtonWrapper>
  );
};
