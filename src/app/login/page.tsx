...
import { useRouter } from 'next/navigation'; // Troque "next/router" por "next/navigation"
...
const LoginPage = () => {
  const router = useRouter(); // Alteração: agora usa o hook "useRouter" de "next/navigation"
  ...
  const decodeToken = (token: string) => {
    try {
      ...
      // Extraia o credencialId do payload
      const credencialId = (decoded as any).credencialId; // Assumindo que credencialId está no payload
      return credencialId;
    } catch (error) {
  ...
  const handleSubmit = async (values: { username: string; password: string }) => {
    try {
      ...
      const credencialId = decodeToken(response.data.token); // Decodifica o token
      localStorage.setItem('credencialId', credencialId);
      console.log("Credencial ID:", credencialId);

      router.push('/posts'); // Redireciona para a página principal
    } catch (error) {
